...
"
  ...
  (define-key ahg-status-mode-map "E" 'ahg-status-diff-ediff) ;; TN: using ediff
...
;; TN: trying to get ediff working copying stuff from psvn.el
(eval-when-compile (require 'ediff-util))

(defun ahg-status-diff-ediff ()
  "Shows changes of the current revision wrt. its parent using ediff. If ALL is t,
shows changes of all marked files. Otherwise, shows changes of
the file on the current line."
  (interactive)
  (let* ((files
	 ;; TN: I'm lazy so I'm copying the same code to get all
	 ;; (marked) files , and checking later that files only holds
	 ;; one element.
	  (let ((n (ewoc-locate ewoc))) (when n (list (ewoc-data n)))))
	 (curdir default-directory)
	 (inhibit-read-only t)
	 (target-file (cddr (car files)))
	 (original-buffer (current-buffer))
	 (buf (get-buffer-create (format "*aHg ediff: base of %s*" target-file)))
	 )
    (with-current-buffer buf
      (setq default-directory (file-name-as-directory curdir)))
    (cond ((null files) (message "aHg ediff: no file selected."))
	  ((> (length files) 1) (message "aHg ediff: several files selected, but ediff works on a single one."))
          (t
           (with-current-buffer buf
             (erase-buffer)
             (ahg-push-window-configuration))
           (ahg-generic-command "cat" 
				(list target-file)
				(lexical-let (
					      (ahg-ediff-target-file target-file)
					      (startup-hook '(ahg-ediff-startup-hook))
					      (original-buffer original-buffer)
					      )
				  (lambda (process status)
				    (if (string= status "finished\n")
					(let (
					      (ahg-ediff-process-buffer (process-buffer process))
					      (ahg-status-original-buffer original-buffer)
					      )
					  (ediff-buffers  ahg-ediff-process-buffer (find-file-noselect ahg-ediff-target-file)
							  startup-hook))
				      (ahg-show-error process))))
				buf)))
    ))
...
(defun ahg-ediff-startup-hook ()
  ;; (message "svn-ediff-startup-hook: ediff-after-quit-hook-internal: %S" ediff-after-quit-hook-internal)
  (add-hook 'ediff-after-quit-hook-internal
	    `(lambda ()
	       (ahg-ediff-exit-hook
		',ahg-ediff-process-buffer ',ahg-status-original-buffer)
	       )
            nil 'local)
  )

(defun ahg-ediff-exit-hook (process-buffer original-buffer)
  (kill-buffer process-buffer)
  (when (and original-buffer
	     (buffer-live-p original-buffer)
	     (not (get-buffer-window original-buffer t)))
    (ignore-errors (switch-to-buffer original-buffer)))
  )
